...
from training.train_sequence import train_sequence
...
def run_experiment(backbone, loader, pt, sess, seq_len, seed, lock, filename):
    ...
    acc, f1, PE, PE_norm = train_sequence(backbone, loader, pt, sess, seq_len, seed)
    cleanup_memory()
...
def main(args):
    ...
    loader = LoadDataSequence(dataset)
    ...
    path = os.path.join(MODEL2SAVE,
                        f"original-sequences/{backbone}/{dataset}")
